...
<script>
	...
	export default {
		...
		methods: {
			load() {
				const { TMap, map, renderByParent, id, enableCollision, styles, geometries } = this
				for (const key in styles) {
					styles[key] = createMarkerStyle(TMap, styles[key])
				}
				let geometries_copy = []
				if (geometries) {
					geometries.map(e => {
				...
				const overlay = new TMap.MultiMarker({
					map: map,
					styles: styles,
				...
				this.$emit('getMultiMarker', overlay)
