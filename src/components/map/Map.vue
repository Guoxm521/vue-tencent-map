...
<script>
	...
	export default {
		...
		watch: {
			center: {
				handler(val, oldVal) {
					const { TMap, map, zoom, center, isPanTo } = this;
					let { lat, lng } = center;
					if (
						val !== oldVal &&
						lng >= -180 &&
						...
						if (isPanTo) {
							map.panTo(createPoint(TMap, center), { duration: 500 });
						} else {
							map.setCenter(createPoint(TMap, center));
						}
					}
				},
				deep: true,
		...
		methods: {
			...
			getCenterPoint() {
				...
				switch (checkType(center)) {
					case "Object":
						return createPoint(TMap, center);
					default:
